 ...
 * the desire's area / type / budget / rooms, ranked by closeness (best first).
...
class MatchDesireToInventory
{
    ...
    public function handle(Desire $desire): Collection
    {
        $units = Unit::query()
            ...
            ->when($desire->rooms_min, fn ($q) => $q->where('rooms', '>=', $desire->rooms_min))
        ...
        // client indicated, then how tightly the room count matches.
        return $units
    ...
    private function score(Unit $unit, Desire $desire): float
    {
        ...
        if ($desire->rooms_min !== null && $unit->rooms !== null) {
            // Prefer the fewest extra rooms over the minimum (weighted to dominate ties).
            $score += ($unit->rooms - $desire->rooms_min) * 1_000_000;
        }
