// Country dialing codes (indicatifs) for phone inputs.
// `favorite: true` entries are pinned to the top of pickers, in the order below.
// The default selected dial code is DEFAULT_DIAL_CODE (+213, Algeria).

export const DEFAULT_DIAL_CODE = '+213'

// Favorites first (order matters), then the rest alphabetically by name.
export const COUNTRY_CODES = [
  // ── Favorites ──────────────────────────────────────────────
  // `groups` = how the national number is grouped for display, e.g. [3, 2, 2, 2]
  // renders "555 04 21 42". When it doesn't match the actual length a generic
  ...
  { iso: 'EG', name: 'Egypt', dial: '+20', flag: '🇪🇬', groups: [3, 3, 4] },
  ...
  { iso: 'DE', name: 'Germany', dial: '+49', flag: '🇩🇪', groups: [3, 4, 4] },
  ...
  { iso: 'IT', name: 'Italy', dial: '+39', flag: '🇮🇹', groups: [3, 3, 4] },
  ...
  { iso: 'MA', name: 'Morocco', dial: '+212', flag: '🇲🇦', groups: [3, 2, 2, 2] },
  ...
  { iso: 'QA', name: 'Qatar', dial: '+974', flag: '🇶🇦', groups: [4, 4] },
  { iso: 'RO', name: 'Romania', dial: '+40', flag: '🇷🇴' },
  { iso: 'RU', name: 'Russia', dial: '+7', flag: '🇷🇺' },
  { iso: 'RW', name: 'Rwanda', dial: '+250', flag: '🇷🇼' },
  { iso: 'SA', name: 'Saudi Arabia', dial: '+966', flag: '🇸🇦', groups: [2, 3, 4] },
  ...
  { iso: 'ES', name: 'Spain', dial: '+34', flag: '🇪🇸', groups: [3, 3, 3] },
  ...
  { iso: 'TN', name: 'Tunisia', dial: '+216', flag: '🇹🇳', groups: [2, 3, 3] },
  { iso: 'TR', name: 'Turkey', dial: '+90', flag: '🇹🇷', groups: [3, 3, 4] },
  { iso: 'TM', name: 'Turkmenistan', dial: '+993', flag: '🇹🇲' },
  { iso: 'UG', name: 'Uganda', dial: '+256', flag: '🇺🇬' },
  { iso: 'UA', name: 'Ukraine', dial: '+380', flag: '🇺🇦' },
  { iso: 'GB', name: 'United Kingdom', dial: '+44', flag: '🇬🇧', groups: [4, 3, 3] },
...
// Unique dial codes, longest first — used to split a stored "+213555..." value
// back into (dial code, national number) when editing.
export const DIAL_CODES_BY_LENGTH = [...new Set(COUNTRY_CODES.map((c) => c.dial))].sort(
  (a, b) => b.length - a.length,
)

// Most countries use a "0" national trunk prefix that is dropped when writing the
// number in international form (e.g. Algeria 0555… → +213555…). A few keep the
// leading 0 as part of the international number — Italy is the notable one — so we
// must NOT strip it for those.
export const KEEP_LEADING_ZERO_DIALS = new Set(['+39'])

// Convert a locally-typed number into its national portion for international
// format: keep digits only and drop the trunk "0" (unless this dial keeps it).
export function toNationalNumber(input, dial) {
  const digits = (input ?? '').replace(/\D/g, '')
  if (KEEP_LEADING_ZERO_DIALS.has(dial)) return digits
  return digits.replace(/^0+/, '')
}

// Generic grouping for countries without an explicit `groups` pattern (or when
// the number length doesn't match it): a leading group of 3 for odd lengths,
// then pairs — e.g. 9 digits → "555 04 21 42".
function defaultGroups(len) {
  if (len <= 4) return [len]
  const groups = []
  let remaining = len
  if (remaining % 2 === 1) {
...
function chunk(digits, sizes) {
  ...
  if (i < digits.length) out.push(digits.slice(i)) // trailing remainder
  return out
}

// Group a national number for display, e.g. ("555042142", "+213") → "555 04 21 42".
export function formatNational(national, dial) {
  const digits = (national ?? '').replace(/\D/g, '')
  if (!digits) return ''
  const country = COUNTRY_CODES.find((c) => c.dial === dial && c.groups)
  const total = country?.groups.reduce((a, b) => a + b, 0)
  const sizes = country && total === digits.length ? country.groups : defaultGroups(digits.length)
  return chunk(digits, sizes).join(' ')
}

// Format a stored value for display, e.g. "+213555042142" → "+213 - 555 04 21 42".
// Values that aren't in international form are returned unchanged.
export function formatPhone(value) {
  const raw = (value ?? '').trim()
  if (!raw.startsWith('+')) return raw
  const compact = raw.replace(/\s+/g, '')
  const dial = DIAL_CODES_BY_LENGTH.find((code) => compact.startsWith(code))
  if (!dial) return raw
  const national = formatNational(compact.slice(dial.length), dial)
